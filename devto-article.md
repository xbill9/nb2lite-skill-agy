---
title: "Teaching Claude Code to Paint: A Stateful Image-Editing Skill Built on Gemini's Interactions API and MCP"
published: false
description: "How nb2lite-skill-claude packages Google's gemini-3.1-flash-lite-image as a Claude Code skill + MCP server — with multi-turn stateful edits, an idiot-proof install guide, and a dogfooded cover image."
tags: ai, claudecode, gemini, mcp
cover_image: https://raw.githubusercontent.com/xbill9/nb2lite-skill-claude/main/devto-cover.jpg
---

> **TL;DR:** [nb2lite-skill-claude](https://github.com/xbill9/nb2lite-skill-claude) wraps Google's `gemini-3.1-flash-lite-image` model (NB2Lite) in a tiny FastMCP server and packages it as a Claude Code skill. You type "generate an image of a cyberpunk kitchen" into Claude Code, and it just... does it. Then you say "add a neon RAMEN sign" and it edits *the same image* without re-prompting the whole scene. Oh, and the cover image of this article? Generated by the thing the article is about — dogfooding all the way down. More on that at the end.
...
2. A **Claude Code skill** (`nb2lite-image`) that teaches Claude *when* and *how* to use those tools well.
...
- Anything else → ask Claude to call `get_help`; it reports the live config.
...
- The **skill is active inside its own repository** — open Claude Code in a clone and the `nb2lite-image` skill and `nb2lite-agent` server are already wired up, so every development session doubles as an integration test.
- The **integration tests** (`make test`) drive the same four MCP tools an end user would, against the live API.
- And now, **the cover image of this article was generated by the exact skill the article describes**, from inside a Claude Code session in this repo. One tool call, first attempt, no retouching:
...
- **Repo:** [github.com/xbill9/nb2lite-skill-claude](https://github.com/xbill9/nb2lite-skill-claude) (Apache-2.0)
