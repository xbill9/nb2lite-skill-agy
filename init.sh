#!/bin/bash
# Local setup for the NB2Lite agent repo: checks python3, installs Python
# dependencies, registers the nb2lite-agent MCP server in this repo's
...
main() {
    ...
    # --- Register the nb2lite-agent MCP server for this repo ---
    ...
    echo -e "\n--- Registering the nb2lite-agent MCP server (.mcp.json) ---"
    if [ -f "$SCRIPT_DIR/.mcp.json" ] && grep -q '"nb2lite-agent"' "$SCRIPT_DIR/.mcp.json"; then
        echo "nb2lite-agent already registered in .mcp.json; leaving it in place."
...
if os.path.exists(path):
    with open(path) as f:
        config = json.load(f)
config.setdefault("mcpServers", {})["nb2lite-agent"] = {
    "command": "python3",
...
with open(path, "w") as f:
    ...
    f.write("\n")
print(f"Registered 'nb2lite-agent' in {path}")
EOF
    then
    ...
    if [ "$ERRORS" -eq 0 ]; then
        ...
        echo "/mcp should list 'nb2lite-agent'. To install the skill into another"
